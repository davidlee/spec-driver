...
- `_find_backlog(root, pattern, kind) -> Iterator[ArtifactRef]`
- `_find_cards(root, pattern) -> Iterator[ArtifactRef]`: Find cards by repo-wide rglob matching {pattern}-\*.md.
- `_find_changes(root, pattern, kind) -> Iterator[ArtifactRef]`
- `_find_decisions(root, pattern) -> Iterator[ArtifactRef]`
- `_find_drift_ledgers(root, pattern) -> Iterator[ArtifactRef]`
- `_find_memories(root, pattern) -> Iterator[ArtifactRef]`
- `_find_plans(root, pattern) -> Iterator[ArtifactRef]`
- `_find_policies(root, pattern) -> Iterator[ArtifactRef]`
- `_find_requirements(root, pattern) -> Iterator[ArtifactRef]`
- `_find_specs(root, pattern) -> Iterator[ArtifactRef]`
- `_find_standards(root, pattern) -> Iterator[ArtifactRef]`
- `_matches_pattern(artifact_id, pattern) -> bool`: Check if artifact ID matches fnmatch pattern (case-insensitive).
- `_normalize_plan_id(raw_id) -> str`: Normalize plan ID: '41' -> 'IP-041', 'IP-041' -> 'IP-041'.
- `_parse_prefix(raw_id) -> <BinOp>`: Extract alphabetic prefix from an ID like 'DE-061' or 'ISSUE-045'.

Returns the prefix string (e.g. 'DE', 'ISSUE') if the ID matches the
PREFIX-NNN pattern, or None otherwise.

- `_resolve_backlog(root, raw_id, kind) -> ArtifactRef`
- `_resolve_card(root, raw_id) -> ArtifactRef`
- `_resolve_change(root, raw_id, kind) -> ArtifactRef`
- `_resolve_decision(root, raw_id) -> ArtifactRef`
- `_resolve_drift_ledger(root, raw_id) -> ArtifactRef`
- `_resolve_memory(root, raw_id) -> ArtifactRef`
- `_resolve_plan(root, raw_id) -> ArtifactRef`
- `_resolve_policy(root, raw_id) -> ArtifactRef`
...
- `__init__(self, artifact_type, artifact_id, paths) -> None`
...
- `__init__(self, artifact_type, artifact_id) -> None`
