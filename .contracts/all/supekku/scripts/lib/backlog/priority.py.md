...
Returns:
Tuple of (prefix, partitions) where: - prefix: Items before the first shown item - partitions: List of (head, [tail_items]) pairs - head: A shown item (or None for trailing unshown items) - tail_items: Unshown items that follow this head
