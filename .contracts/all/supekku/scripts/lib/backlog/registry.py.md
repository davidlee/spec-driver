...
- `__init__(self) -> None`
- `_load(self) -> None`: Scan filesystem and cache all backlog items.
- `_parse_item(self, md_file, kind_dir_name) -> <BinOp>`: Parse a single backlog item file into a BacklogItem.
