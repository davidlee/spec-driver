...
- `_prepare_requirement_row(req) -> list[str]`: Prepare a single requirement row with styling.
- `_prepare_requirement_tsv_row(req) -> list[str]`: Prepare a single requirement as a plain TSV row.
- `_requirement_column_widths(show_external) -> Callable[Tuple[List[int], dict[Tuple[int, int]]]]`: Return a column-width calculator for the requirement table layout.
