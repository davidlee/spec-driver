...
- `query_forward(graph, artifact_id) -> list[GraphEdge]`: Return edges where *artifact_id* is the source.
...
- `query_inverse(graph, artifact_id) -> list[GraphEdge]`: Return edges where *artifact_id* is the target.
