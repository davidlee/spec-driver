...
- `_find_all_packages(root) -> Iterator[Path]`: Find all Python packages under root (internal helper).

Args:
root: Root directory to search

Yields:
Paths to all directories containing **init**.py

- `_is_relative_to(path, base) -> bool`: Check if path is relative to base (Python 3.9+ compatible).
...
(have **init**.py and no child packages).
...
Examples: >>> packages = find_all_leaf_packages(Path("supekku")) >>> len(packages)
16 >>> Path("supekku/scripts/lib/formatters") in packages
True
...
containing an **init**.py file (a Python package).
...
Examples: >>> file_path = Path("supekku/scripts/lib/formatters/change_formatters.py") >>> find_package_for_file(file_path)
Path("supekku/scripts/lib/formatters") >>> find_package_for_file(Path("some_script.py"))
None
...
1. Contains an **init**.py file (is a Python package)
...
Examples: >>> is_leaf_package(Path("supekku/scripts/lib/formatters"))
True >>> is_leaf_package(Path("supekku/scripts/lib")) # Has child packages
False
...
ValueError: If path doesn't contain **init**.py

Examples: >>> validate_package_path(Path("supekku/cli")) # OK >>> validate_package_path(Path("nonexistent"))
FileNotFoundError: Package path does not exist: nonexistent >>> validate_package_path(Path("some_file.py"))
