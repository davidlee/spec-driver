...
- `__init__(self) -> None`
- `_populate(self, node, directory, depth) -> None`: Add files and directories, skipping hidden files and symlinks.
- `_select_primary(self) -> None`: Cursor the primary file node after population.
- `_sort_key(self, path) -> tuple[Tuple[int, int, str]]`: Sort: directories first, primary file first among files, then alpha.
- `_walk_nodes(self) -> list[TreeNode[Path]]`: Flat list of all visible nodes for cursor indexing.
