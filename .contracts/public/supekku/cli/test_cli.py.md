...
### TestFilterBackwardCompatibility
...
- `test_list_adrs_single_status(self)`: Single-value -s flag on list adrs still works.
- `test_list_deltas_json_schema_unchanged(self)`: JSON output for deltas includes expected fields.
- `test_list_deltas_no_filters(self)`: list deltas with no filters returns results. - -- Empty/no filters return all --
- `test_list_deltas_regexp_filter(self)`: Regexp filter on list deltas still works. - -- Regex filters unchanged --
- `test_list_deltas_single_status(self)`: Single-value -s flag on list deltas still works. - -- Single-value status filters (all list commands) --
- `test_list_requirements_json_schema_unchanged(self)`: JSON output schema includes expected fields. - -- New flags don't interfere with existing output --
- `test_list_requirements_no_filters(self)`: list requirements with no filters returns results. - Should have output
- `test_list_requirements_regexp_filter(self)`: Regexp filter on list requirements still works.
- `test_list_requirements_single_kind(self)`: Single-value --kind FR on list requirements still works.
- `test_list_requirements_single_status(self)`: Single-value --status on list requirements still works.
- `test_list_specs_no_filters(self)`: list specs with no filters returns results.
- `test_list_specs_single_kind(self)`: Single-value -k flag on list specs still works.
- `test_spec_filter_with_kind(self)`: Combining --spec with --kind still works.
- `test_status_with_regexp(self)`: Combining --status with --regexp still works. - -- Combined old + new flags --
...
- @pytest.mark.parametrize(cmd,flag, List[Tuple[List[list, deltas], --implements], Tuple[List[list, requirements], --verified-by], Tuple[List[list, specs], --informed-by]]) `test_list_help_documents_reverse_flags(self, cmd, flag) -> None`: List command help should document reverse relationship flags.
- @pytest.mark.parametrize(cmd, List[List[list, deltas], List[list, specs], List[list, requirements], List[list, adrs]]) `test_list_help_has_examples(self, cmd) -> None`: Each list command help should include usage examples.
- @pytest.mark.parametrize(cmd, List[List[list, deltas], List[list, specs], List[list, requirements]]) `test_list_help_shows_multi_value_syntax(self, cmd) -> None`: List command help should document multi-value filter syntax.
...
- `test_list_audits_json_equals_format_json(self)`: Test --json produces same output as --format=json for audits.
- `test_list_audits_json_flag(self)`: Test list audits accepts --json flag.
...
- `test_list_audits_basic(self)`: Test list audits returns successfully.
- `test_list_audits_filter_substring(self)`: Test list audits --filter substring matching.
- `test_list_audits_invalid_format(self)`: Test list audits with invalid format exits with failure.
- `test_list_audits_invalid_regexp(self)`: Test list audits with invalid regexp exits with failure.
- `test_list_audits_regexp(self)`: Test list audits --regexp filtering. - empty result is not an error
- `test_list_audits_status_filter(self)`: Test list audits --status filter.
- `test_list_audits_tsv_format(self)`: Test list audits --format tsv.
...
- `test_list_audit_singular_alias(self)`: Test list audit singular alias works.
- `test_list_audits_help(self)`: Test list audits command help.
...
- `test_vkind_flag_exists(self)`: Test that list requirements accepts --vkind flag.
- `test_vkind_multi_value(self)`: Test --vkind with comma-separated values (OR logic).
- `test_vkind_single_value(self)`: Test --vkind with single kind filters by verification kind.
- `test_vstatus_and_vkind_combined(self)`: Test combining --vstatus and --vkind (AND logic across flags).
- `test_vstatus_flag_exists(self)`: Test that list requirements accepts --vstatus flag.
- `test_vstatus_multi_value(self)`: Test --vstatus with comma-separated values (OR logic).
- `test_vstatus_nonexistent_returns_empty(self)`: Test --vstatus with value matching no entries returns empty.
- `test_vstatus_single_value(self)`: Test --vstatus with single value filters by verification status.
- `test_vstatus_with_spec_filter(self)`: Test combining --vstatus with --spec filter.
- `test_vstatus_with_verified_by_filter(self)`: Test combining --vstatus with --verified-by filter.
