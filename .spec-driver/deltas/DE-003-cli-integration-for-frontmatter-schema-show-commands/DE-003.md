...
applies_to:
  specs: [PROD-004]
  requirements: [PROD-004.FR-004]
outcome_summary: "Enable agents and developers to query frontmatter JSON Schema via CLI commands. All 13 frontmatter kinds accessible through `spec-driver schema show frontmatter.*` commands with multiple output formats (json-schema, yaml-example)."
risk_register:
  - id: DE-003.RISK-01
