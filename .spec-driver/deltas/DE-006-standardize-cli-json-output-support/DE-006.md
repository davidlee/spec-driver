...
All CLI commands support consistent --json output for machine-readable results
...
entries:
- type: document
  id: ISSUE-007 documentation and evidence
- type: document
  id: Existing --json implementation in create spec command
- type: document
  id: Agent workflow requirements from PROD-001 and PROD-002
```
...
risks:
- id: RISK-001
  title: Breaking changes to existing text output parsers
  likelihood: low
  impact: medium
  mitigation: Add --json as opt-in flag; preserve default text output unchanged
  status: mitigated
- id: RISK-002
  title: Inconsistent JSON schema across commands
  likelihood: medium
  impact: low
