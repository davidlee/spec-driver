...
- **Risk**: Ambiguous ID matches (multiple `T123-*.md`) – _Likelihood_: medium – _Impact_: medium – _Mitigation_: make ambiguity explicit; print candidates; provide `--anywhere` only as an expansion.
- **Risk**: Over-aggressive template rewriting – _Likelihood_: low – _Impact_: high – _Mitigation_: rewrite only the first H1 and `Created:` line; test against template variants.
- **Risk**: Cards not always in `kanban/` – _Likelihood_: medium – _Impact_: low – _Mitigation_: `find card` is repo-wide; `show card` supports `--anywhere`.
