...
context_inputs: []
applies_to:
  specs: [PROD-010, SPEC-110]
  requirements: [PROD-010.FR-005]
---
...
### P3 — Domain reference adapters (prerequisite for P4/P5)

12. Dedicated domain-field collectors for Decision, Policy/Standard, Requirement, BacklogItem
13. `_collect_from_domain_fields()` dispatcher chaining all collectors into `collect_references()`
14. Update P1-8 backlog `--related-to` to use new collector

### P4 — Generic reverse reference filtering

15. Generic `--referenced-by <type>` / `--not-referenced-by <type>` flags on all list commands
16. Convenience aliases: `list deltas --unaudited`, `list requirements --unimplemented`
17. Shared `load_all_artifacts()` helper in `cli/common.py`

### P5 — Graph/neighbourhood view

18. `show <kind> <id> --related` for spec, delta, requirement, issue (primary navigation anchors)
...
- **Risk**: Performance of reverse lookups on large workspaces – _Likelihood_: low – _Impact_: medium – _Mitigation_: Lazy evaluation; existing registry indices
- **Risk**: Scope creep across tiers – _Likelihood_: medium – _Impact_: low – _Mitigation_: Each P-tier is a separate phase; ship incrementally
...
### 2026-03-13 — Phase 01 complete

**Done**: All P0 tasks (1.1–1.4) implemented with tests.

- 1.1: `_format_relations()` key fix (`kind` → `type`)
- 1.2: `_format_spec_relations()` + `_format_requirements_summary()` in spec_formatters; show.py caller passes counts
- 1.3: `discover_plans()` ValueError catch + stderr warning
- 1.4: `Spec.to_dict()` includes `relations` when present

**Verification**: `just` passes — 3869 tests, ruff clean, pylint 9.72/10 (no new warnings).

**Findings**:

- `Spec.relations` does `dict(r)` preserving extra keys (e.g. `annotation`). `to_dict()` passes these through — consistent with model's dict-based serialisation. Not a problem but worth knowing for JSON consumers.
- `FrontmatterValidationResult` stores relations as list of Mapping, not typed objects. The `.relations` property filters to entries with both `type` and `target`.

**Commits**: 33680a3

**Next**: Phase 02 — create phase sheet, implement 5 relational filter flags (DR-090 §P1).

### 2026-03-13 — Phase 02 complete

**Done**: All P1 filter flags (2.1–2.5) implemented with 15 tests.

- 2.1: `list audits --delta <ID>` — normalize_id + relations target match
- 2.2: `list revisions --delta <ID>` — same pattern
- 2.3: `list requirements --implemented-by <ID>` — single delta lookup, parse relationships block, filter by implements list
- 2.4: `list deltas --spec <ID>` — match applies_to.specs + relations targets
- 2.5: `list backlog --related-to <ID>` — frontmatter.relations target match

**Verification**: `just` passes — 3884 tests, ruff clean, pylint 9.72/10.

**Next**: Phase 03 — P2 richer show output (requires DR revision first).
