...
12. Generic `--referenced-by <type>` / `--not-referenced-by <type>` flags on all list commands
13. Convenience aliases: `list deltas --unaudited`, `list requirements --unimplemented`
14. BacklogItem `.relations` property synthesising from `linked_deltas`/`related_requirements`
