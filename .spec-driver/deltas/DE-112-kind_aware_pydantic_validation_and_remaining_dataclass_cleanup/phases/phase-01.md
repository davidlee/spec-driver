---
id: IP-112-P01
slug: "112-kind_aware_pydantic_validation-phase-01"
name: "Phase 01 — Convert FrontmatterValidationResult and Relation"
created: "2026-03-22"
updated: "2026-03-22"
status: draft
kind: phase
plan: IP-112
delta: DE-112
objective: >-
  Convert FrontmatterValidationResult and Relation from @dataclass(frozen=True)
  to Pydantic BaseModel(frozen=True). Update validate_frontmatter() call site.
entrance_criteria:
  - DR-112 approved
  - Existing core tests passing
exit_criteria:
  - Both classes are Pydantic BaseModel with frozen=True
  - validate_frontmatter() constructs FrontmatterValidationResult via Pydantic
  - All existing tests passing
  - Lint clean
---

# Phase 01 — Convert FrontmatterValidationResult and Relation

## Tasks

| Status | ID  | Description | Notes |
| ------ | --- | ----------- | ----- |
| [ ]    | 1.1 | Convert Relation to frozen BaseModel | field(default_factory=dict) → dict default |
| [ ]    | 1.2 | Convert FrontmatterValidationResult to frozen BaseModel | tuple fields stay as tuple |
| [ ]    | 1.3 | Update validate_frontmatter() call site | Direct construction still works |
| [ ]    | 1.4 | Fix TYPE_CHECKING imports if needed | Path import gotcha from mem.gotcha.pydantic.migration |
| [ ]    | 1.5 | Run tests and lint | Core tests + specs tests (Spec uses FrontmatterValidationResult) |

### Notes

- `Relation.type` shadows `type` builtin — Pydantic handles this fine
- `FrontmatterValidationResult.data` is `Mapping[str, Any]` (immutable) — keep as-is
- `FrontmatterValidationResult.relations` is `tuple[Relation, ...]` — Pydantic handles nested frozen models
- Tests that check frozen mutation will need `ValidationError` instead of `AttributeError` (per mem.gotcha.pydantic.migration)
