...
- `2026-05-16` — **C4 landed**: kwarg-threading collapse propagated to `_apply_spec_relationships`. Test-file placement: `spec_metadata_test.py` (not the §3.4 `relationships_metadata_test.py` name) per the `<source>_metadata_test.py` mirror rule. Capabilities wrapper accepts `spec_id` for API symmetry but `del`'s it; phase-sheet §6's "no `spec` field at data layer" claim was imprecise — the metadata definition has a required `spec` field; the wrapper's restraint is about preserving the DE-118 invariant (no legacy enforcement existed), not about data shape. Hoisted `extract_relationships` + `validate_spec_relationships` from inner imports to `sync.py` module-top during the migration to avoid net pylint regression.
- `2026-05-16` — **C5 landed**: `RevisionBlockValidator` + `_disallow_extra_keys` + `ValidationMessage` retired. Added `validate_revision_change(data)` wrapper despite phase-03 §3.5 prescribing direct `MetadataValidator(...)` swaps — the wrapper caches the validator instance (matching C3/C4 pattern) and centralises the two call sites. Signature **diverges** from sibling wrappers: takes parsed `data` dict, not block. Reason: `RevisionChangeBlock` parses on demand (no cached `.data`), and `changes/updater.py` validates a mutable dict in place across an intervening mutation — calling `block.parse()` post-mutation would reset the change. No `revision_id` kwarg (no legacy ID-equality semantic to preserve; the `validate_spec_capabilities` `del`-pattern is only justifiable when symmetry buys something). Kwarg-collapse footprint **zero** this round — `_apply_revision_blocks` constructed the validator inline, not via a threaded kwarg, so there was nothing to collapse. `HAND_ROLLED_ADAPTERS` now empty; harness still runs (metadata-only smoke pass) — OQ-HARNESS-LIFECYCLE deferred to P04 per §3.6 default (option a).
- `2026-05-16` — **P04 hand-off recorded**: phase-03 §3.6 wrap-up still owes the IP-118 §9 progress tick and the harness final-state line in notes.md. Open queue for P04: `REVISION_BLOCK_JSON_SCHEMA` deletion (revision.py:28 + the 4 buggy regex patterns) → tests `test_metadata_generates_json_schema` may rebase onto a metadata-only schema generator; `_entry_shape` replacement; OQ-NAMING-COLLISIONS rename; OQ-HARNESS-LIFECYCLE keep/decommission/repurpose decision.
...
- **C2 finding** (2026-05-11): the plan trio reinforced the C1 pattern — all three classes (`PlanOverview`, `PhaseOverview`, `PhaseTracking`) had zero production callers. **Two of the first two swap commits, four of the seven retiring validators, had no production callers.** The C3–C5 inventory should re-verify (`RevisionBlockValidator` and `DeltaRelationshipsValidator` are confirmed to have callers per P01 §3.1, §3.2), but the pattern is now: validate production-caller existence first, then choose between delete-only and loader-swap-plus-wrappers shapes.
