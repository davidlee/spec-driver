...
- `2026-05-17` — **OQ-HARNESS-LIFECYCLE (4.6) — option (a) KEEP.** `supekku/scripts/lib/blocks/metadata/snapshot_compare.py` remains as runnable infrastructure with `HAND_ROLLED_ADAPTERS={}` (the P03-final state). Rationale:
  1. **Future-schema-change diagnostic value** — running the harness after extending or modifying any `*_metadata.py` declaration catches corpus drift before it lands in the test suite. This is concrete value beyond DE-118; the module earns its keep as a metadata-only smoke check.
  2. **Decommissioning is irreversible** (R14). Recovery requires recreating from git history; the lifecycle decision is asymmetric.
  3. **STD-004 ownership clarified**: owner = blocks-metadata subsystem; re-run trigger = after extending/modifying `*_metadata.py` declarations, or as a one-shot diagnostic against the live `.spec-driver/` corpus. Both documented in the module docstring.
  - Option (b) decommission rejected: no compelling reason (the `HAND_ROLLED_ADAPTERS={}` state is harmless and the metadata-only pass is independently useful).
  - Option (c) repurpose-into-`just check` rejected: the 245-LOC harness walks the entire corpus (~1657 files at HEAD), too heavy for the per-commit gate. The on-demand trigger captures the use case without slowing the default check.
  - Module docstring updated to remove the "OQ-HARNESS-LIFECYCLE: settled before delta closure" line and to declare owner + re-run trigger; `HAND_ROLLED_ADAPTERS` comment updated to drop the P03/P04 transitional framing.
- **__all__ audit (4.6 part A)**: AST sweep across `supekku/scripts/lib/**` and `spec_driver/**` found zero stale entries — `replace_all` on the 4.4 / 4.5 renames updated `__all__` lines atomically. No additional cleanup commit needed.
