...
- **Ship client-upgrade memory** (user request 2026-05-30, deferred to close): author `mem.signpost.spec-driver.upgrade.metadata-blocks-0-10` (tag `spec-driver`) in root `memory/` (hatch force-includes `memory/` → `supekku/memory` in the wheel, so it installs to client projects). Content = `admin migrate` runbook + per-kind strict-mode flip sequence + drift-ledger residual model. Deferred because the residual-disposition decision (DL-049..074 backfill vs accept) is itself Phase 4 work; runbook must document the settled state. Existing memories cover migration *mechanics* (builder-facing); this is the missing *operator/upgrade* runbook. See IP-136 §8 Phase 4 exit criteria.
