...
summary: "CLI event emission uses a process-boundary wrapper in main() to emit events after every command, rather than scattering explicit emit_event() calls in each command function. result_callback is unsound because all commands exit via raise typer.Exit(), which bypasses it."
---
