...
summary: In this repo, prefer frequent, small `.spec-driver` commits and a clean working tree; when `.spec-driver` changes
  and code changes both exist, commit them together or separately based on what naturally goes out first.
priority:
  severity: high
  weight: 8
scope:
