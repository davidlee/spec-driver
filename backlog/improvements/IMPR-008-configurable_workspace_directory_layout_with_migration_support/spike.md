...
1. **How many path references exist across code, tests, docs, and config?**
   DE-044 centralized constants but the *values* still assume top-level dirs.
...
test data). Changing constant *values* in `paths.py` would propagate cleanly
...
artifacts (e.g. `specify/decisions/ADR-001-...`). These are *prose references*,
