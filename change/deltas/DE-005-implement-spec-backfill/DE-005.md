...
- Future: Support for non-Python languages (Go, TypeScript)
...
- Add `show template` command to `supekku/cli/show.py`
- Implement `supekku/cli/backfill.py` module
- Implement `supekku/scripts/lib/specs/completion.py` logic
...
- **Q3**: CLI command naming: `backfill` vs `complete` vs `fill`? **Decided**: `backfill`
...
1. **Stub detection**: Exact string match of content after frontmatter vs. rendered template
