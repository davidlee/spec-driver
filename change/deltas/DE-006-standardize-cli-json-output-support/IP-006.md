...
phases:
  - id: IP-006.PHASE-01
    ...
    objective: >-
      Extend --json flag to create, list, and show commands with standardized output format
    entrance_criteria:
      - Delta approved
...
entries:
  - artefact: VT-CLI-JSON-001
    ...
    status: planned
    notes: Unit tests for each command's JSON output format
  - artefact: VT-CLI-JSON-002
    kind: VT
    requirement: DE-006.agent-workflow
    phase: IP-006.PHASE-01
    status: planned
    notes: Integration test for agent workflow using JSON output
  - artefact: VT-CLI-JSON-003
    kind: VT
    requirement: DE-006.backward-compat
    phase: IP-006.PHASE-01
    status: planned
    notes: Regression tests ensuring text output unchanged
```
