...
phases:
  - id: IP-007.PHASE-01
    name: Phase 01 - Coverage registry integration
    objective: >-
      Drive verification coverage entries into the requirements registry and
      surface lifecycle drift warnings during sync.
    entrance_criteria:
      - SPEC-122 guidance reviewed
    exit_criteria:
      - Lifecycle fields update when coverage status changes
      - Validation warning emitted for spec vs audit/plan drift
  - id: IP-007.PHASE-02
    name: Phase 02 - Completion workflow enforcement
    objective: >-
      Enforce coverage updates when completing deltas and document the
      workflow for agents.
    entrance_criteria:
      - Phase 01 exit criteria met
    exit_criteria:
      - `spec-driver complete delta` blocks when coverage missing
      - RUN.md and CLAUDE command docs updated
```
