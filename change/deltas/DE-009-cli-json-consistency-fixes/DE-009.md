...
- type: implements
  target: PROD-010
  description: Implements Priority 1 requirements for CLI agent UX consistency
applies_to:
  specs:
  - SPEC-110
  requirements:
...
# Lint as you go
just lint
just pylint supekku/cli/list.py

# Test specific command changes
...
3. **JSON formatter consistency**:
   - Review existing JSON formatters in `spec_formatters.py`
   - Maintain `{"items": [...]}` wrapper structure
   - Include same metadata fields as other artifacts
...
**Testing Strategy**:
- Write tests FIRST (TDD approach per project conventions)
- Test JSON parseability: `json.loads(output)` must succeed
- Test schema stability: Compare before/after JSON structure
- Test status filter: All valid values, empty results, invalid values
- Test help text: Automated checks for documented flags
