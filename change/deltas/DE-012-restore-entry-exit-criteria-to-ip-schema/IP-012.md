...
phases:
  - id: IP-012.PHASE-01
    ...
    objective: "Restore entry/exit criteria fields to plan.overview schema and update create_phase to copy criteria from IP to phase sheets"
    entrance_criteria:
...
entries:
  - artefact: VT-SCHEMA-013-001
    ...
    status: planned
    notes: Schema validation tests in plan_metadata_test.py
  ...
  - artefact: VT-CREATE-013-002
    ...
    phase: IP-012.PHASE-01
    status: planned
    notes: Phase creation with criteria copy tests in creation_test.py
  ...
  - artefact: VT-COMPAT-013-001
    ...
    phase: IP-012.PHASE-01
    status: planned
    notes: Backward compatibility tests - existing suite plus ID-only specific tests
```
