...
Change the adapter interface so that adapters write contract files to caller-specified canonical paths under `.contracts/`. The caller (`process_source_unit`) computes per-variant output file paths using the existing mirror mapping logic from `mirror.py`, applied *before* generation rather than after. Python uses a staging directory with post-generation distribution.
