...
def load_workflow_config(
  repo_root: Path, known_kinds: set[str] | None = None
) -> dict:
  ...
  Args:
    ...
    known_kinds: Optional set of registered artefact kinds. When
      provided, unknown keys in ``[validation.strict]`` trigger a
      warning (F-47). When ``None`` (default), no warning is emitted
      — suitable for infra config reads that should not warn on user
      kind configuration.
  ...
  _warn_unknown_strict_kinds(merged, known_kinds)
  return merged
...
def _warn_unknown_strict_kinds(
  config: dict, known_kinds: set[str] | None = None
) -> None:
  ...
  When ``known_kinds`` is ``None`` (default), no warning is emitted —
  the caller has not supplied a kind registry. This is deliberate:
  strict-kind warnings fire only at validating entrypoints, not on
  every infra config read.
  """
...
def get_strict_map(config: dict, known_kinds: set[str]) -> dict[str, bool]:
  ...
  return {
    kind: bool(value)
    for kind, value in strict_section.items()
