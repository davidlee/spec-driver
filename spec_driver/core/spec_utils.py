...
`dump_markdown_file_update(...)` and `write_markdown_file(...)` are the
canonical core write primitives. `dump_markdown_file_create(..., kind=)` lives
in `spec_driver.orchestration.templates` (kind-aware rendering is orchestration
concern per POL-003 / DEC-128-004).
"""
...
def write_markdown_file(path: Path, fm_yaml: str, body: str) -> None:
  """Write a markdown file with frontmatter and body (atomic).

  Pure core primitive — takes already-rendered YAML frontmatter text.
  Kind-aware rendering is orchestration's concern; core owns write mechanics.

  Args:
    path: Target path.
    fm_yaml: Rendered YAML frontmatter text (no leading/trailing ``---``).
...
def dump_markdown_file_update(
  ...
  map. Atomic via `write_markdown_file`. No kind awareness — comment hints come
  from the existing file, not metadata.
  """
  path = Path(path)
