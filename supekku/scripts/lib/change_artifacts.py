    ...
    if kind == "delta":
        ...
        if block and not DeltaRelationshipsValidator().validate(
            ...
            if primary_specs:
                ...
                existing.update(
                    str(item) for item in primary_specs if isinstance(item, str)
                )
                if existing:
        ...
        for remaining in phase_lookup.values():
            remaining = remaining.copy()
            remaining.setdefault("phase", remaining.get("id"))
            phases_data.append(remaining)
