...
def check_deps(ws: Workspace) -> list[DiagnosticResult]:
  ...
  results.append(_check_binary(
    "go", "Go toolchain",
    "Install Go from https://go.dev/dl/",
  ))
  results.append(_check_binary(
    "gomarkdoc", "Go doc generator",
...
def _check_binary(
  name: str, label: str, install_hint: str
) -> DiagnosticResult:
...
def _check_ts_doc_extract(ws: Workspace) -> DiagnosticResult:
  if is_npm_package_available(
    "ts-doc-extract", package_root=ws.root
  ):
    return DiagnosticResult(
      category=CATEGORY,
      name="ts-doc-extract",
