        ...
        if functions_to_show:
            ...
            for func in functions_to_show:
                ...
                if func.get("decorators"):
                    decorators = (
                        " ".join([f"@{dec}" for dec in func["decorators"]]) + " "
                    )

                docstring_info = (
                    f": {func['docstring']}" if func.get("docstring") else ""
                )
            ...
            if methods_to_show:
                ...
                for method in methods_to_show:
                    ...
                    if method.get("decorators"):
                        decorators = (
                            " ".join([f"@{dec}" for dec in method["decorators"]]) + " "
                        )

                    docstring_info = (
                        f": {method['docstring']}" if method.get("docstring") else ""
                    )
                    comment_info = (
                        f" - {method['comment']}" if method.get("comment") else ""
                    )
