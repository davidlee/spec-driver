...
class RevisionBlockValidator:
    ...
    def _check_root(
        self, data: dict[str, Any], messages: list[ValidationMessage],
    ) -> None:
        ...
        if requirements is None:
            ...
            for index, requirement in enumerate(requirements):
                self._validate_requirement(
                    requirement, messages, path=("requirements", index),
                )
    ...
    def _validate_metadata(
        ...
        if "revision" not in metadata:
            ...
            if not isinstance(revision, str) or not _is_revision_id(revision):
                messages.append(
                    ValidationMessage(
                        ("metadata", "revision"), "must match pattern RE-###",
                    ),
                )
    ...
    def _validate_requirement(
        ...
        *,
        ...
        if requirement_id is not None and (
            ...
            or not _is_requirement_id(requirement_id)
        ):
            messages.append(
                ValidationMessage(
        ...
        if lifecycle is not None:
            if not isinstance(lifecycle, dict):
                ...
                if introduced_by is not None and (
                    not isinstance(introduced_by, str)
                    or not _is_revision_id(introduced_by)
                ):
                    messages.append(
                        ValidationMessage(
...
def extract_revision_blocks(
    markdown: str, *, source: Path | None = None,
